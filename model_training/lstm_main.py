...
print(
    "Number of Embedder Parameters: "
    + str(sum([p.numel() for p in embedder.parameters()]))
)
print(
    "Number of LSTM Parameters: "
    + str(sum([p.numel() for p in lstm.parameters()]))
)
print(
    "Number of MLP Parameters: "
    + str(sum([p.numel() for p in mlp.parameters()]))
)
logging.info(f"Number of Model Parameters: {number_of_parameters}")
logging.info(
    "Number of Embedder Parameters: "
    + f"{sum([p.numel() for p in embedder.parameters()])}"
)
logging.info(
    "Number of LSTM Parameters: "
    + f"{sum([p.numel() for p in lstm.parameters()])}"
)
logging.info(
    "Number of MLP Parameters: "
...
for epoch in pbar:  # Number of epochs
    ...
    if val_loss < best_loss:
        best_loss = val_loss
        best_lstm_model = copy.deepcopy(
            lstm.state_dict()
        )  # Save the best model
        best_embedder_model = copy.deepcopy(embedder.state_dict())
        best_mlp_model = copy.deepcopy(mlp.state_dict())
        patience_counter = 0
    else:
        patience_counter += 1
...
if best_model is not None:
    lstm.load_state_dict(best_lstm_model)  # Load the best model
    embedder.load_state_dict(best_embedder_model)
    mlp.load_state_dict(best_mlp_model)

torch.save(lstm.state_dict(), model_name + "_lstm.pth")
torch.save(embedder.state_dict(), model_name + "_embedder.pth")
torch.save(mlp.state_dict(), model_name + "_mlp.pth")
